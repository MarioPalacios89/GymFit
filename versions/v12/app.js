...
function renderDia(idx) {
    ...
    html += (dia.ejercicios || []).map((ex, i) => {
        ...
        return `
            <div class="card exercise-card p-6 relative ${accentClass} ${energyMeta.card || ''}" style="--card-order:${i}" id="card-${i}" data-day="${idx}" data-ex="${i}" data-save-selected="false" data-ex-name="${String(active.nombre || ex.nombre || '').replace(/"/g, '&quot;')}">
