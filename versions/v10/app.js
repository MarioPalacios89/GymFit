...
function getSetGuidance(set = {}, ex = {}) {
    if (isWarmupSet(set)) {
        const pct = set.porcentaje_carga ? `${set.porcentaje_carga} de la carga` : 'Carga progresiva';
        const reps = set.repeticiones || 'sin fatiga';
        return `${pct} · ${reps} reps${set.opcional ? ' · opcional' : ''}`;
    }
    const pieces = [];
    if (set.rpe !== null && set.rpe !== undefined && String(set.rpe).trim() && String(set.rpe).toUpperCase() !== 'N/A') pieces.push(`RPE ${set.rpe}`);
    if (set.tecnica) pieces.push(set.tecnica);
    if (!pieces.length && ex.repeticiones) pieces.push(`${ex.repeticiones} reps`);
    return pieces.join(' · ');
}
        ...
        <div class="hero-panel mb-6">
            <div class="hero-top">
                ...
                <div class="min-w-0 flex-1">
                    ...
                    <p class="hero-sub">${(day?.ejercicios || []).length} ejercicios · ${(day?.ejercicios || []).reduce((acc, ex) => acc + getExerciseSetPlan(ex).filter(s => !isWarmupSet(s)).length, 0)} efectivas · ${(day?.ejercicios || []).reduce((acc, ex) => acc + getExerciseSetPlan(ex).filter(isWarmupSet).length, 0)} aproximación</p>
                </div>
            </div>
...
async function loadRoutineDb(week = semanaActiva) {
...
function initializeRoutineUi() {
    ...
    document.getElementById('sub-meta').innerText = `${db?.version || `Semana ${semanaActiva}`} · ${objetivo}`.replace(/_/g, ' ');
...
function renderDia(idx) {
    ...
    html += (dia.ejercicios || []).map((ex, i) => {
        ...
        const rows = setPlan.map((set, setIndex) => {
            ...
            return `
                <div class="exercise-set-row-wrapper ${warmup ? 'warmup-set' : 'work-set'} ${set.opcional ? 'optional-set' : ''}">
                    ...
                    <div class="set-guidance">
                        ...
                        <span>${guidance || 'Técnica controlada'}</span>
                    </div>
                </div>
            `;
        }).join('');
            ...
            <div class="card exercise-card p-6 relative ${accentClass} ${energyMeta.card || ''}" id="card-${i}" data-day="${idx}" data-ex="${i}" data-ex-name="${String(active.nombre || ex.nombre || '').replace(/"/g, '&quot;')}">
                <div class="exercise-card-head mb-2">
                    <div class="exercise-card-main">
                        ...
                        <div class="exercise-card-content min-w-0 flex-1"><h3 class="exercise-name">${active.nombre}</h3></div>
                    </div>
